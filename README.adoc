...
|`dirs` |The comma-separated root directories used to recursively search for files. |The current execution directory `.`.
|`include` |The comma-separated include patterns used to search for files. The value of the include patterns are always additive to the default file patterns. |Typical Markdown and AsciiDoc files: `*.md,*.adoc`
